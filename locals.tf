locals {
  ...
  management_lb_ipv4           = local.cluster_loadbalancer_running ? data.hcloud_load_balancers.rke2_management.load_balancers[0].ipv4 : hcloud_load_balancer.management_lb.ipv4
  ...
  cluster_host                 = "https://${local.management_lb_ipv4}:6443"
